...
exports.getEventData = regionalFunctions.https.onCall(async (data, context) => {
  ...
  if (!eventId) {
    ...
    eventData.phases = await Promise.all(phasesSnapshot.docs.map(async (phaseDoc) => {
      const enigmasSnapshot = await phaseDoc.ref.collection("enigmas").orderBy("id").get(); // Ordenar para consistência
...
// --- Função para ações do enigma (LÓGICA DE PROGRESSÃO ATUALIZADA) ---
exports.handleEnigmaAction = regionalFunctions.https.onCall(async (data, context) => {
    ...
    // Lógica para getStatus e purchaseHint (mantida)
    if (action === "getStatus") {
    ...
    if (action === "purchaseHint") {
        if (phaseOrder > 5) { throw new functions.https.HttpsError("failed-precondition", "Dicas não disponíveis."); }
        await playerRef.set({ events: { [eventId]: { hintsPurchased: admin.firestore.FieldValue.arrayUnion(phaseOrder) } } }, { merge: true });
    ...
    if (action === "validateCode") {
        ...
        if (isCorrect) {
            ...
            if (currentIndex < enigmas.length - 1) {
                // Se houver um próximo enigma nesta fase
            ...
            await playerRef.set({
                events: { [eventId]: { currentPhase: nextPhaseOrder, currentEnigma: nextEnigmaIndex } }
            }, { merge: true });

            return { success: true, message: "Parabéns! Código correto.", nextStep: nextStep };
        } else {
            // Lógica de tentativas incorretas
