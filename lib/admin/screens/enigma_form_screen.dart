    ...
    if (enigma != null) {
      ...
      if (enigma.location != null) {
        _selectedLocation = LatLng(
          enigma.location!.latitude,
          enigma.location!.longitude,
        );
        _markers = {
          Marker(
            ...
            onPressed: () async {
              ...
              try {
                final result = await SaverGallery.saveImage(
                  ...
                  quality: 100,
                  fileName: "qr_code_${widget.enigma!.id}",
                  skipIfExists: false,
                );
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text("Salvo na galeria: $result")),
                  );
                }
              } catch (e) {
                if (context.mounted) {
                      ...
                      child: ClipRRect(
                        ...
                        child: GoogleMap(
                          initialCameraPosition: CameraPosition(
                            target:
                                _selectedLocation ??
                                const LatLng(-23.5505, -46.6333), // Default SP
                            zoom: 15,
                          ),
                          gestureRecognizers: {
