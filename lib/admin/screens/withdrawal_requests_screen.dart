...
class _WithdrawalRequestsScreenState extends State<WithdrawalRequestsScreen> {
  ...
  Future<void> _handleApprove(String id, String userName, double amount) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          "Deseja aprovar o saque de R\$ ${amount.toStringAsFixed(2)} para $userName?\nIsso marcará a transação como paga.",
  ...
  Future<void> _handleReject(String id, String userName, double amount) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              "Deseja rejeitar o saque de R\$ ${amount.toStringAsFixed(2)} para $userName?\nO valor será estornado para a conta do usuário.",
              style: const TextStyle(color: secondaryTextColor),
            ),
    ...
    if (confirm == true) {
      try {
        await _firebaseService.rejectWithdrawal(
          id,
