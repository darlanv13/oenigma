...
class _EnigmaScreenState extends State<EnigmaScreen>
  ...
  void initState() {
    ...
    _shakeDisposer = reaction<bool>(
      (_) => _store.shakeTrigger,
      (trigger) {
    ...
    _cooldownDisposer = reaction<String?>(
      (_) => _store.cooldownUntil,
      (cooldownUntil) {
    ...
    _fundsDisposer = reaction<bool>(
      (_) => _store.insufficientFunds,
      (funds) {
        if (funds) {
  ...
  Future<void> _handleNextStep(Map<String, dynamic> nextStep) async {
    switch (nextStep['type']) {
      case 'event_complete':
        final double prizeWon = (nextStep['prizeWon'] as num?)?.toDouble() ?? 0.0;
        final List<PhaseModel> allPhases = await _firebaseService.getPhasesForEvent(widget.event.id);
        if (mounted) {
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                final UserWalletModel walletData = await _firebaseService.getUserWalletData();
                if (!mounted) return;
  ...
  Widget _buildEnigmaContent() {
    ...
    switch (enigma.type) {
      ...
      case 'text':
        return Column(
          children: [
            if (enigma.imageUrl != null &&
                enigma.imageUrl!.isNotEmpty)
              ClipRRect(
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
                      ...
                      Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (context) => ScannerScreen(
                            onScan: (scannedCode) {
                              _store.handleAction('validateCode', widget.event.id, widget.phase.order, code: scannedCode);
                            },
                          ),
                        ),
                      );
              ...
              label: Text(
                _store.isBlocked
                    ? 'Aguarde o Cooldown'
                    : (_store.isNear ? 'ESCANEAR CÓDIGO' : 'APROXIME-SE DO LOCAL'),
              ),
              style: ElevatedButton.styleFrom(
        ...
        child: TextButton.icon(
          onPressed: _store.isLoading
              ...
              : () async {
                  ...
                  if (confirmed == true) {
                    _store.handleAction('purchaseHint', widget.event.id, widget.phase.order);
                  }
                },
